...
from pepkio_mendelian_cross_solver.config import TOOL_ID
from pepkio_mendelian_cross_solver.exceptions import PepkioAPIError
from pepkio_mendelian_cross_solver.models import RunResult
...
def test_get_manifest_and_cache(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert m1["tool_id"] == "mendelian-cross-solver"
        assert m1 is m2
    assert len(calls) == 1
...
def test_list_examples_and_get_example_input(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert inp["dominance_mode"] == "complete"
        assert inp["parent1_genotype"] == "Aa"
        assert inp["parent2_genotype"] == "Aa"
...
def test_run_sends_correct_request(mock_manifest, mock_run_response):
    ...
    with _make_client(handler) as client:
        result = client.run(client.get_example_input("complete_monohybrid"), label="pytest")

    assert captured["path"] == f"/api/tools/v1/tools/{TOOL_ID}/run"
    assert captured["auth"] == "Bearer test-key"
    assert captured["body"]["input"]["dominance_mode"] == "complete"
    assert captured["body"]["options"]["label"] == "pytest"
    assert result.status == "completed"
    assert result.result["phenotype_ratios"]["ratio"] == "3:1"
